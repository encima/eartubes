...
def movie_search():
    ...
    query = "(SELECT id, title, production_year AS year FROM title WHERE UPPER(title) LIKE UPPER(\"" + term  + "\") AND kind_id=1 LIMIT 10) UNION ALL (SELECT id, title, production_year AS year FROM title WHERE UPPER(title) LIKE UPPER(\"%" + term  + "%\") AND kind_id=1 LIMIT 10);" 
